// ═══════════════════════════════════════════════════════════════════
//  RIOT MD  ·  MESSAGE HANDLER  (IMPROVED STATUS HANDLING)
// ═══════════════════════════════════════════════════════════════════
...
// ── Extract caption from status message ──────────
function extractStatusCaption(msg) {
  const m = msg.message;
  if (!m) return null;
...
async function getContactName(sock, number) {
  try {
  ...
  return null; // Return null if not found, will use number as fallback
}
...
// ── STATUS HANDLER ────────────────────────────────
// IMPROVED: Better caption and contact name handling
export async function statusHandler(sock, msg, userId) {
  ...
  let senderName = senderNumber; // fallback to number
  
  try {
    // Try to get name from contacts
    const contactName = await getContactName(sock, sender);
    if (contactName) {
      senderName = contactName;
    }
  } catch (e) {
    // If error, just use number
    senderName = senderNumber;
  }

  // Extract caption from status
  const caption = extractStatusCaption(msg);
  ...
  if (settings.autoviewstatus) {
    ...
    console.log(`  👀  Auto-viewing status from ${senderName}`);
    await sock.readMessages([msg.key]).catch(() => {});
  }
  ...
  if (settings.autoreactstatus && sender) {
    ...
    console.log(`  🔥  Auto-reacting to status from ${senderName}`);
  ...
  if (settings.autosavestatus && hasMedia) {
    try {
      const stream    = await downloadContentFromMessage(msg.message[mediaType === 'image' ? 'imageMessage' : 'videoMessage'], mediaType);
      const chunks    = [];
      for await (const chunk of stream) chunks.push(chunk);
      const buf       = Buffer.concat(chunks);
      ...
      console.error(`  ❌  Error saving status: ${e.message}`);
    }
  }

  // Anti-delete status: save status before it gets deleted
  // IMPROVED: Now captures caption and contact name correctly
  if (settings.antideletestatus && hasMedia) {
    try {
      ...
      let deleteCaption = `🛡️ *Anti-Delete Status*\n👤 From: ${senderName} (${senderNumber})`;
      if (caption) deleteCaption += `\n📝 Caption: ${caption}`;
      ...
      console.log(`  🛡️   Anti-delete status captured from ${senderName}`);
    } catch (e) {
      console.error(`  ❌  Error in anti-delete status: ${e.message}`);
    }
  }
}
