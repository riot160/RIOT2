...
export async function createSession(userId, phoneNumber = null, pairingMode = true) {
  ...
  //  FIXED: Now properly resets presence when OFF
  // ────────────────────────────────────────────────
  const presenceInterval = setInterval(async () => {
    ...
    if (settings.alwaysonline) {
      // Send available presence every 30 seconds to maintain online status
      await sock.sendPresenceUpdate('available').catch(() => {});
    } else {
      // When disabled, periodically reset to unavailable to allow normal WhatsApp presence
